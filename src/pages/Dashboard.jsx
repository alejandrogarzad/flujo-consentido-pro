...
import { fmtMXN, calcularTotalTerapia, calcularTotalEvento, paramsToObject, MESES, estatusCxC } from "@/lib/calculos";
import { TrendingUp, TrendingDown, DollarSign, Wallet, AlertCircle, Users } from "lucide-react";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      base44.entities.SesionMensual.list(),
  ...
  // Calcular ingresos cobrados por mes (monto_pagado sin IVA, tal como está en BD)
  const ingresosPorMes = Array(12).fill(0);
  pagos.forEach(p => {
    if ((!p.anio || p.anio === 2026) && p.mes) {
  ...
  eventos.forEach(ev => {
    if (ev.monto_pagado) {
      ...
      if (d.getFullYear() === 2026) {
        ingresosPorMes[d.getMonth()] += Number(ev.monto_pagado);
      }
    }
  });
  subarr.forEach(s => {
    if ((!s.anio || s.anio === 2026) && s.mes) {
  ...
  gastos.forEach(g => {
    ...
    if (d.getFullYear() === 2026) egresosPorMes[d.getMonth()] += Number(g.monto || 0);
  });
  // Agrupar nómina por mes: sumar sueldos, aguinaldo y vacaciones directamente de BD
  // IMSS e ISN se calculan sobre sueldo_transferencia (sin factor adicional — ya es el valor neto real)
  nomina.forEach(n => {
    if (n.anio && n.anio !== 2026) return;
    const idx = (n.mes || 1) - 1;
    if (idx >= 0 && idx < 12) {
  ...
  // Solo mostrar meses hasta el mes actual (datos reales)
  const mesActual = new Date().getMonth(); // 0-indexed, abril = 3
  ...
  // Saldo acumulativo (solo meses con datos)
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold text-stone-800 mb-6">Dashboard Ejecutivo 2026</h1>
              ...
              {chartData.map((row, i) => {
                ...
                return (
                  <tr key={i} className={`border-t border-stone-50 ${esFuturo ? "opacity-30" : "hover:bg-stone-50/50"}`}>
                    ...
                    <td className="px-4 py-3 text-right text-green-600">{esFuturo ? "—" : fmtMXN(row.ingresos)}</td>
                    <td className="px-4 py-3 text-right text-red-500">{esFuturo ? "—" : fmtMXN(row.egresos)}</td>
                    <td className={`px-4 py-3 text-right font-medium ${!esFuturo && row.ingresos - row.egresos >= 0 ? "text-green-600" : "text-red-500"}`}>
                      {esFuturo ? "—" : fmtMXN(row.ingresos - row.egresos)}
                    </td>
                    <td className={`px-4 py-3 text-right font-bold ${!esFuturo && row.saldo >= 0 ? "text-violet-600" : "text-red-600"}`}>
                      {esFuturo ? "—" : fmtMXN(row.saldo)}
                    </td>
                  </tr>
                );
              })}
